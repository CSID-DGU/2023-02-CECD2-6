...
def full_selection(doc_sent_list, abstract_sent_list, summary_size=3):
    ...
    selected_idx3_list = greedy_selection(
...
class BertData:
    ...
    def preprocess(
            self, src_list, use_bert_basic_tokenizer=False, is_test=False
        ):
            """
            ext에서는 sent_labels만 사용되고, tgt_subtoken_idxs는 만들어지긴 하는데 결국 사용되지는 않음.
            """

            if (not is_test) and len(src_list) == 0:
                return None

            original_src_txt = [" ".join(s) for s in src_list]
            ...
            _sent_labels = [0] * len(src_list)
            #for l in sent_labels_list:
            ##    _sent_labels[l] = 1

            src_list = [src_list[i][: self.args.max_src_ntokens_per_sent] for i in idxs]
            ##sent_labels_list = [_sent_labels[i] for i in idxs]

            src_list = src_list[: self.args.max_src_nsents]
            ##sent_labels_list = sent_labels_list[: self.args.max_src_nsents]

            if (not is_test) and len(src_list) < self.args.min_src_nsents:
                return None

            src_txt = [" ".join(sent) for sent in src_list]
            ...
            ##sent_labels_list = sent_labels_list[: len(cls_ids)]

            #tgt_subtokens_str = (
            #    self.tgt_bos
            #    + " "
            #    + f" {self.tgt_sent_split} ".join(
            #        [" ".join(self.tokenizer.tokenize(" ".join(tt))) for tt in tgt_list]
            #    )
            #    + " "
            #    + self.tgt_eos
            #)
            ...
            #tgt_txt = "<q>".join([" ".join(tt) for tt in tgt_list])
...
def _format_to_bert(params):
    ...
    torch.save(datasets, 'output_val.pt')
